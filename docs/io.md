...
| container\_registry\_config | Configuration for Azure Container Registry. | <pre>object({<br>    sku                       = optional(string)<br>    quarantine_policy_enabled = optional(bool)<br>    zone_redundancy_enabled   = optional(bool)<br>  })</pre> | <pre>{<br>  "quarantine_policy_enabled": false,<br>  "sku": "Premium",<br>  "zone_redundancy_enabled": true<br>}</pre> | no |
